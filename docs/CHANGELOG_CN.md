...
### 鏂板
- **鏂板涓庢彃浠朵竴涓€瀵瑰簲鐨勫師鐢?Native Debug API**
  - 鏂板 `native.get_xrefs` 涓?`native.get_xref_count`锛岀敤浜庝氦鍙夊紩鐢ㄦ煡璇?
  - 鏂板 `native.list_patches` 涓?`native.get_patch_at`锛岀敤浜?Patch 鏋氫妇涓庢煡璇?
  - 鏂板 `native.enum_handles` 涓?`native.enum_tcp_connections`锛岀敤浜庢灇涓惧彲瑙佸彞鏌勪笌 TCP 杩炴帴
- **鍐呭瓨椤甸潰淇濇姢鎺у埗**
  - 鏂板 `memory.set_protection`锛屽彲閫氳繃鎻掍欢淇敼鍐呭瓨椤甸潰淇濇姢灞炴€?
- **瀹屾暣 Python MCP Client**
  - 灏?`x64dbg-mcp.py` 浠庢渶灏忕ず渚嬭剼鏈崌绾т负瀹屾暣鍙敤鐨?MCP client
  - 鏂板涓?`x64dbg-example.py` 瀵归綈鐨勭ず渚嬮鏍?wrapper 鍑芥暟
  - 鏀寔 `python x64dbg-mcp.py RegisterGet rip` 杩欑被鐩存帴宸ュ叿椋庢牸璋冪敤

### 鍙樻洿
- **鎻掍欢涓?Client 鑳藉姏瀵归綈**
  - `x64dbg-mcp.py` 鐜板湪鍙澶栨毚闇查偅浜涚敱鎻掍欢鐪熷疄鎵垮姏鐨?wrapper 鍑芥暟
  - 灏?`XrefGet`銆乣XrefCount`銆乣GetPatchList`銆乣GetPatchAt`銆乣EnumHandles`銆乣EnumTcpConnections` 閲嶆柊鎺ュ洖鐪熷疄鎻掍欢鏂规硶锛屼笉鍐嶆槸鍗犱綅瀹炵幇
- **鏉冮檺涓庢枃妗ｆ洿鏂?*
  - 榛樿鏂规硶鐧藉悕鍗曚笌杩愯鏃舵彃浠堕厤缃腑鍧囨柊澧?`native.*`
  - `README.md`銆乣README_CN.md` 鍜?`QUICKSTART_CN.md` 宸插悓姝ユ洿鏂帮紝鍐欐槑鏂板鎻掍欢鑳藉姏涓?Python client 鍔熻兘

### 淇
- **鏋勫缓涓庤繍琛屽彲闈犳€ф敼杩?*
  - 淇 `PatchEnum` 璋冪敤鏂瑰紡锛屾敼涓烘纭殑涓ゆ寮忔灇涓炬祦绋?
  - 閫氳繃鍔犲叆甯﹁寖鍥存鏌ョ殑 `uint64_t -> duint` 杞崲锛屼慨澶?x86 涓嬬殑缂栬瘧璀﹀憡
  - 鍦?native debug handler` 鏂囦欢灞€閮ㄥ睆钄?SDK 寮曞叆鐨?`C4324` 瀵归綈璀﹀憡
  - 淇 `XrefGet`锛氬湪娌℃湁浜ゅ弶寮曠敤鐨勫湴鍧€涓婅繑鍥炵┖缁撴灉锛岃€屼笉鍐嶆姤閿?
- **楠岃瘉**
  - 閲嶆瀯寤恒€侀儴缃插苟閲嶅惎鎻掍欢鍚庯紝宸插畬鎴愭柊澧?native API 鐨勭鍒扮楠岃瘉
  - 宸茬‘璁?`XrefGet`銆乣XrefCount`銆乣GetPatchList`銆乣GetPatchAt`銆乣EnumHandles`銆乣EnumTcpConnections` 杩愯鏃舵垚鍔?
